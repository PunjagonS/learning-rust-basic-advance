...
fn main() -> Result<(), ureq::Error> {
    ...
    let now = Instant::now(); // Start time watch.

    for web_page in &webpages {
        let web_body = agent.get(web_page).call()?.into_string()?;
    }
    println!("Time taken wihtout Threads: {:.2?}", now.elapsed()); // Stop time watch and print.
    ...
    println!("Time taken using Threads: {:.2?}", now.elapsed()); // Stop time watch and print.
    Ok(())
}
